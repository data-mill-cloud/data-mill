...
if [ -z "$ACTION" ] || [ "$ACTION" != "install" ] && [ "$ACTION" != "delete" ];then
	...
	helm upgrade $cfg__mlflow__release data-mill/mlflow \
	 ...
	 --set server.artifacturi="s3://mlflow",server.s3endpoint="http://${cfg__mlflow__artifact_store__minio_release}:9000",server.accesskey="${ACCESS_KEY}",server.secretkey="${SECRET_KEY}" \
         --install --force
else
